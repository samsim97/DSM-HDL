...
- Big picture
  - This repo implements a first-order Delta-Sigma DAC in synthesizable Verilog. The main signal flow is: delta_feedback -> integrator -> quantizer. See `src/rtl/top_dsm_dac_order_1.v` for the composed top-level module.
...
- Key files and locations (start here)
  - Top-level RTL: `src/rtl/top_dsm_dac_order_1.v`
  - Building blocks: `src/rtl/delta_feedback.v`, `src/rtl/integrator.v`, `src/rtl/quantizer.v`
  - Testbenches: `src/tb/` (look at `tb_top_dsm_dac_order_1.v`)
...
- Project-specific conventions and patterns
  - Narrow, composable modules: prefer one clear responsibility per file (e.g., integrator only accumulates signed samples when `i_sample` asserted).
...
- Test and simulation workflow (what works here)
  - Testbenches live under `src/tb/`. The primary top testbench is `src/tb/tb_top_dsm_dac_order_1.v` which declares a large `r_input_signal` array of fixed-width samples.
  ...
  - Example quick simulation (typical):
    - Compile: `iverilog -g2012 -o tb_sim.exe src/rtl/*.v src/tb/tb_top_dsm_dac_order_1.v`
    - Run: `vvp tb_sim.exe`
    (Adjust for your simulator of choice; the design is standard Verilog-2001/2012 compatible.)
  - For waveform inspection, add `$dumpfile("dump.vcd"); $dumpvars(0, tb_top_dsm_dac_order_1);` to the testbench and open `dump.vcd` in `gtkwave`.
